...
class ParsrClient:
    ...
    def send_document(
        ...
        if document_name not in self.revision_history:
            ...
            latest_revision = max(
                VersionInfo.parse(i)
                for i in list(self.revision_history[document_name].keys())
            )
            if revision == "major":
    ...
    def get_document_name_from_request_id(self, request_id: str) -> str:
        ...
        for document_name in list(self.revision_history.keys()):
            if request_id in [
                self.revision_history[document_name][i]
                for i in list(self.revision_history[document_name])
            ]:
                return document_name
        return ""

    def compare_revisions(
        self, document_name: str, revisions: list = [], pretty_html: bool = False
    ) -> list:
